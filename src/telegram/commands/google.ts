...
/**
 * Registers all /google_* Telegram command handlers.
 *
 * Commands:
 *   /google_setup    — upload client_secret.json (Desktop credential)
 *   /google_connect  — start OAuth flow (sends auth URL)
 *   /google_complete — finish OAuth flow (paste failed redirect URL)
 *   /google_status   — show current auth state
 *   /google_disconnect — remove stored credentials
 */
...
/**
 * Routes a /google_* command to the appropriate handler.
  ...
  try {
    ...
    await ctx.reply(
      `✓ Google client credentials saved${projectNote}.\n\n` +
        "Run /google_connect your@gmail.com to authorize your Google account."
    );
...
async function handleGoogleConnect(
  ...
  if (!hintEmail || !EMAIL_RE.test(hintEmail)) {
    await ctx.reply(
      "Usage: /google_connect your@gmail.com\n\n" +
  ...
  if (!creds) {
    await ctx.reply(
      "No Google client credentials found. Run /google_setup first, " +
...
async function handleGoogleComplete(
  ...
  if (!raw) {
    await ctx.reply("Usage: /google_complete <URL copied from browser address bar>");
    return;
  }

  log.info(
    { subsystem: "google", event: "complete_attempt", url: redactQueryParams(raw) },
    "google_complete called"
  );
  ...
  if (!consumed) {
    await ctx.reply(
      "✗ This link has expired, was already used, or was not generated by this bot. " +
        "Run /google_connect to start over."
    );
    return;
  }
  ...
  const creds = clientStore.read();
  if (!creds) {
    await ctx.reply("✗ Client credentials missing. Run /google_setup first.");
    return;
  }
...
async function handleGoogleStatus(ctx: Context, deps: GoogleCommandDeps): Promise<void> {
  ...
  await ctx.reply(
    `Client credentials: ${hasClient ? "✓ uploaded" : "✗ missing — run /google_setup"}\n` +
      `Account authorization: ${
        hasCreds
          ? `✓ connected${email ? ` as ${email}` : ""}`
          : "✗ not connected — run /google_connect"
      }`
  );
}
...
async function handleGoogleDisconnect(ctx: Context, deps: GoogleCommandDeps): Promise<void> {
  ...
  await ctx.reply(
    "Google account disconnected. Client credentials are kept — " +
      "run /google_connect to re-authorize."
  );
}
